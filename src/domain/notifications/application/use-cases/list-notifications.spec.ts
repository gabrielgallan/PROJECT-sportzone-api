...
describe('List notifications use case', () => {
	...
	it('should be able to list paginated notifications', async () => {
		...
		expect(result.notifications.data).toHaveLength(2);
		expect(result.notifications.meta.page).toBe(1);
		expect(result.notifications.meta.limit).toBe(2);
		expect(result.notifications.meta.total).toBe(3);
	});
});
